...
export const OrderDetailModal: React.FC<OrderDetailModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-40 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-6 overflow-y-auto">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl w-full max-w-3xl overflow-hidden my-auto max-h-[92vh] flex flex-col">
        {/* Modal Header */}
        <div className="px-6 py-4 bg-slate-50 dark:bg-slate-800/60 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-3 py-1.5 rounded-xl bg-brand-50 dark:bg-brand-950 text-brand-600 dark:text-brand-300 hover:bg-brand-100 text-xs font-semibold flex items-center gap-1.5 transition-colors"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="p-4 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-2">
              ...
              <div className="flex items-center gap-2 text-slate-700 dark:text-slate-300 text-xs">
                ...
                <span>Expected Delivery: <strong>{new Date(order.expectedDeliveryDate).toLocaleDateString('en-GB')}</strong></span>
              </div>
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-slate-800 dark:text-slate-200">
                  {order.items.map((item, idx) => (
                    <tr key={idx}>
                      ...
                      <td className="py-2.5 px-3 text-brand-600 dark:text-brand-400">{item.serviceName}</td>
          ...
          <div>
            {order.remainingBalance > 0 && (
              <button
                ...
                className="px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-semibold text-xs shadow-md shadow-emerald-600/20 flex items-center gap-1.5"
              >
                <CreditCard className="w-4 h-4" />
                <span>Record Payment ({currencySymbol}{order.remainingBalance})</span>
              </button>
            )}
          </div>
          <button
