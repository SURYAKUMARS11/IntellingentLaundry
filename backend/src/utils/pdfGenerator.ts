...
export const generateInvoicePDFBuffer = async (order: any, setting?: any): Promise<Buffer> => {
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.fontSize(9).fillColor('#64748b').font('Helvetica').text(`Date: ${new Date(order.orderDate).toLocaleDateString('en-GB')}`, 380, 100, { align: 'right' });
      doc.text(`Status: ${order.status.toUpperCase()}`, 380, 115, { align: 'right' });
      ...
      order.items.forEach((item: any, index: number) => {
        ...
        doc.text(`${item.itemName} (${item.serviceType})`, 80, startY + 7, { width: 240 });
        doc.text(`${item.quantity}`, 330, startY + 7, { width: 40, align: 'center' });
        doc.text(`₹${item.unitPrice.toFixed(2)}`, 380, startY + 7, { width: 70, align: 'right' });
        doc.text(`₹${item.itemTotal.toFixed(2)}`, 465, startY + 7, { width: 80, align: 'right' });
      ...
      doc.text(`₹${(order.subtotal || 0).toFixed(2)}`, summaryLeft + 100, startY, { width: 95, align: 'right' });

      if (order.discount > 0) {
        startY += 15;
        doc.text('Discount:', summaryLeft, startY, { width: 100, align: 'left' });
        doc.text(`- ₹${order.discount.toFixed(2)}`, summaryLeft + 100, startY, { width: 95, align: 'right' });
      }

      if (order.taxAmount > 0) {
        startY += 15;
        doc.text(`Tax (${order.taxPercent}%):`, summaryLeft, startY, { width: 100, align: 'left' });
        doc.text(`₹${order.taxAmount.toFixed(2)}`, summaryLeft + 100, startY, { width: 95, align: 'right' });
      }
      ...
      doc.text(`₹${order.totalAmount.toFixed(2)}`, summaryLeft + 90, startY + 3, { width: 95, align: 'right' });

      startY += 30;
      doc.fontSize(9).fillColor('#334155').font('Helvetica');
      doc.text('Advance Paid:', summaryLeft, startY, { width: 100, align: 'left' });
      doc.text(`₹${(order.advancePaid || 0).toFixed(2)}`, summaryLeft + 100, startY, { width: 95, align: 'right' });

      startY += 15;
      doc.font('Helvetica-Bold').fillColor(order.remainingBalance > 0 ? '#dc2626' : '#16a34a');
      doc.text('Balance Due:', summaryLeft, startY, { width: 100, align: 'left' });
      doc.text(`₹${(order.remainingBalance || 0).toFixed(2)}`, summaryLeft + 100, startY, { width: 95, align: 'right' });
